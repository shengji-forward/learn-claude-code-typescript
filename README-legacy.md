> Archived: this is the previous README of the TypeScript edition, preserved here before the strict-mirror-to-Python refactor.
...
A teaching repository for implementers who want to build a high-completion coding-agent harness from scratch, written in TypeScript.
...
- knows basic TypeScript / JavaScript
- understands functions, classes, async/await, and Promises
...
The English docs are intended to stand on their own. The chapter order, bridge docs, and mechanism map are aligned with the Python original, so you can follow the same learning path.
...
4. Read `s_full.ts` last, after the mechanisms already make sense separately.
...
├── agents/              # runnable TypeScript reference implementations per chapter
├── docs/en/             # English documentation
├── skills/              # skill files used in s05
├── web/                 # web teaching platform (Next.js)
└── package.json
```

## TypeScript vs Python

This is a TypeScript port of the [original Python repository](https://github.com/shareAI-lab/learn-claude-code). Key differences:

- All agent files use TypeScript with async/await instead of Python's synchronous patterns
- File operations use `fs/promises` (async) instead of `pathlib` (sync)
- Threading is replaced with `worker_threads` for background tasks and teammate workers
- Type annotations and interfaces for compile-time safety
- Node.js event loop instead of Python threads for the cron scheduler
- `npx ts-node` for running sessions instead of `python`

The teaching content, mechanism order, and conceptual explanations are identical to the Python version.
