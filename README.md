...
# Learn Claude Code -- Harness Engineering for Real Agents (TypeScript Edition)

## Agency Comes from the Model. An Agent Product = Model + Harness.

Before we talk about code, let's get one thing straight.

**Agency -- the ability to perceive, reason, and act -- comes from model training, not from external code orchestration.** But a working agent product needs both the model and the harness. The model is the driver, the harness is the vehicle. This repo teaches you how to build the vehicle in TypeScript.
...
This TypeScript edition is a 0->1 learning project for harness engineering -- building the environment that surrounds an agent model. It is a strict mirror of the latest Python mainline (s01-s12).
...
Earlier drafts of this TypeScript edition explored seven additional mechanisms (permissions, hooks, memory, prompt assembly, error recovery, cron, MCP). Those are archived under `agents/extras/` and `docs/extras/en/` -- see the Appendix at the bottom of this README.
...
npm run s:full         # Capstone: all 12 mechanisms combined
```
...
|   +-- extras/                    # TS-only extras archived from an earlier 19-session layout
|-- docs/en/                       # Mental-model-first documentation (English only)
|   +-- extras/en/                 # Archived TS-only chapter and bridge docs
...
Available in [English](./docs/en/) only for this TypeScript edition.
...
Follow on X: [shareAI-Lab](https://x.com/baicai003)
...
## Appendix: TypeScript-Edition Extras (not in Python mainline)

This TypeScript edition previously explored seven additional harness mechanisms that are NOT part of the Python mainline (s01-s12). They are preserved under `agents/extras/` and documented under `docs/extras/en/` for reference, but they are not part of the recommended learning path.

| Extra | Motto | Run | Docs |
|---|---|---|---|
| Permission System | *A safety gate before execution* | `npm run extras:permission` | [s07-permission-system.md](./docs/extras/en/s07-permission-system.md) |
| Hook System | *Extension points around the loop* | `npm run extras:hook` | [s08-hook-system.md](./docs/extras/en/s08-hook-system.md) |
| Memory System | *Durable cross-session knowledge* | `npm run extras:memory` | [s09-memory-system.md](./docs/extras/en/s09-memory-system.md) |
| System Prompt | *Section-based prompt assembly* | `npm run extras:sysprompt` | [s10-system-prompt.md](./docs/extras/en/s10-system-prompt.md) |
| Error Recovery | *Continuation and retry branches* | `npm run extras:recovery` | [s11-error-recovery.md](./docs/extras/en/s11-error-recovery.md) |
| Cron Scheduler | *Time-based triggers* | `npm run extras:cron` | [s14-cron-scheduler.md](./docs/extras/en/s14-cron-scheduler.md) |
| MCP & Plugin | *External capability routing* | `npm run extras:mcp` | [s19-mcp-plugin.md](./docs/extras/en/s19-mcp-plugin.md) |

The TS-only 19-mechanism capstone is preserved at `agents/extras/s_full_all19.ts` and runnable via `npm run extras:full-all19`. The current mainline `agents/s_full.ts` composes only the 12 Python-mainline mechanisms.

Additional TS-only bridge and overview docs (architecture overview, query control plane, tool control plane, runtime task model, MCP capability layers, entity map, team-task-lane model, teaching scope) are archived under [docs/extras/en/](./docs/extras/en/).
