...
**Step 1.** Define the schedule record. Each job stores a cron expression (a compact time-matching syntax like `0 9 * * 1` meaning "9:00 AM every Monday"), the prompt to execute, whether it recurs or fires once, and a `lastFiredAt` timestamp to prevent double-firing.
...
```typescript
create(cronExpr: string, prompt: string, recurring: boolean = true): ScheduleJob {
...
**Step 4.** When a schedule matches, enqueue a notification. The `lastFiredAt` field is updated to prevent the same minute from triggering the job twice.
...
- Prevent double-firing with `lastFiredAt` tracking
