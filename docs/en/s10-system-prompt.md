...
class SystemPromptBuilder {
    build(): string {
        ...
        parts.push(this.buildSkills());
        parts.push(this.buildMemory());
        parts.push(this.buildClaudeMd());
        parts.push(this.buildDynamic());
        return parts.filter(p => p).join("\n\n");
    }
}
```

That is the central idea of the chapter. Each `build*` method pulls from one source only: `buildTools()` reads the tool list, `buildMemory()` reads the memory store, and so on. If you want to know where a line in the prompt came from, you check the one method responsible for it.
