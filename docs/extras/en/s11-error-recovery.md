...
function chooseRecovery(stopReason: string | null, errorText: string | null): RecoveryDecision {
    ...
    const transientWords = ["timeout", "rate", "unavailable", "connection"];
    if (errorText && transientWords.some(word => errorText.includes(word))) {
...
**Step 4. Handle compaction (context overflow).** When the prompt becomes too large, the problem is not the task itself -- the accumulated context needs to shrink before the next turn can proceed. You call the same `autoCompact` mechanism from s06 to summarize history, then retry:
